...
bool ObjectMgr::IsGideonClassQuestAllowed(uint32 questId, uint8 race, uint8 playerClass) const
{
    ...
    // RACE_NONE is not a race, and 1u << (race - 1) would then shift by 255:
    // undefined behaviour rather than a wrong answer, which is the one failure
    // mode that cannot be reasoned about afterwards. The upper bound keeps the
    // shift inside a uint32 should the enum ever grow past the commented-out
    // races that follow RACE_DRAENEI.
    //
    // A logged-in character always has a valid race, so this guards a corrupt
    // row or a caller that does not exist yet, at the cost of one comparison.
    if (race == RACE_NONE || race > RACE_DRAENEI)
        return false;
