...
void Player::UpdateManaRegen()
{
    ...
    // Drinks are mana-regeneration auras which are interrupted when the player
    // stands up. Keep their contribution separate so a server can make resting
    // valuable without accelerating normal spirit, gear or aura regeneration.
    ...
    // Normal flat mana regeneration includes gear and non-drink auras.
    float normal_regen_mp5 =
        (GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_POWER_REGEN, POWER_MANA) +
         m_baseManaRegen) / 5.0f - drink_regen_mp5;
    ...
    SetStatFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER + AsUnderlyingType(POWER_MANA),
        (normal_regen_mp5 + CalculatePct(power_regen, modManaRegenInterrupt)) * ManaIncreaseRate +
        drink_regen_mp5 * ManaDrinkIncreaseRate);

    SetStatFloatValue(UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER + AsUnderlyingType(POWER_MANA),
        (normal_regen_mp5 + power_regen) * ManaIncreaseRate +
        drink_regen_mp5 * ManaDrinkIncreaseRate);
}
